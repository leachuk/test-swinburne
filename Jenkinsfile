pipeline {
  ...
  stages {
    ...
    stage('CacheClearPublish-1') {
      steps {
        sh 'curl -u ${TEST_AUTHOR_CREDS_USR}:${TEST_AUTHOR_CREDS_PSW} -X POST  http://13.238.172.197:4503/etc/acs-commons/dispatcher-flush/disp-flush/_jcr_content/configuration.flush.html'
      }
    }
    stage('CacheClearPublish-2') {
      steps {
        sh 'curl -u ${TEST_AUTHOR_CREDS_USR}:${TEST_AUTHOR_CREDS_PSW} -X POST  http://13.237.162.203:4503/etc/acs-commons/dispatcher-flush/disp-flush/_jcr_content/configuration.flush.html'
      }
    }
  }
}
